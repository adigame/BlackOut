...
if(_val > life_atmfric) exitWith {hint localize "STR_ATM_NotEnough"};
_tax = [_val] call life_fnc_taxRate;
if((_val + _tax) > life_atmfric) exitWith {hint format[localize "STR_ATM_SentMoneyFail",_val,_tax]};

life_atmfric = life_atmfric - (_val + _tax);
