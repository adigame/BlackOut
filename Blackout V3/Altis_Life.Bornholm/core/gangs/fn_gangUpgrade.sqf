...
if(_action) then {
	if(life_atmfric < _upgradePrice) exitWith {
		hint parseText format[
			...
			[life_atmfric] call life_fnc_numberText,
			[_upgradePrice - life_atmfric] call life_fnc_numberText
		];
	};
	__SUB__(life_atmfric,_upgradePrice);
