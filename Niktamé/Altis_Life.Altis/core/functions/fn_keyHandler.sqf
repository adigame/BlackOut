...
switch (_code) do
{
	...
	*/
	  ...
	  if(_shift && playerSide == civilian && !isNull cursorTarget && cursorTarget isKindOf "Man" && isPlayer cursorTarget && alive cursorTarget && cursorTarget distance player < 4 && speed cursorTarget < 1) then
	  {
	   if((animationState cursorTarget) != "Incapacitated" && (currentWeapon player == primaryWeapon player OR currentWeapon player == handgunWeapon player) && currentWeapon player != "" && !life_knockout && !(player getVariable["restrained",false]) && !life_istazed) then
	   {
		...
		if("ItemRadio" in assignedItems cursorTarget) then {
		  ...
		  hint "Le téléphone de la personne est tombé à terre.";
		  _defenceplace1 = "Item_ItemRadio" createVehicle (player modelToWorld[0,0,0]);}
		 else { hint "La personne que vous avez assommée n'a pas de téléphone!"};
	   };
	  };
	};
	...
	case 20:
	{
		...
		{
			if(vehicle player != player && alive vehicle player && !(player getVariable["restrained",false]) && (vehicle player == player)) then
			{
	...
	case 21:
	{
		if(!_alt && !_ctrlKey && !dialog && !(player getVariable ["restrained", false]) && !life_istazed) then
		{
			[] call life_fnc_p_openMenu;
		};
	};
	...
	case 41:
	{
		if(!dialog && !(player getVariable ["restrained", false]) && !life_istazed) then
		{
			[] call life_fnc_p_openMenu;
		};
	};
	
	//TAB Key (Pickaxe shortcut)
	case 15:
	{
		if (vehicle player == player && !(player getVariable ["restrained", false]) && (animationState player) != "Incapacitated" && !life_istazed && !life_action_gathering && life_is_alive) then {
	...
	//We are going to kick in ass alt f4ers jim ! :D
	case 62:
    {
