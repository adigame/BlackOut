...
if (life_is_processing) exitWith {hint "Vous êtes déjà en train de traiter !";};    
...
if(vehicle player != player) exitwith {hint format["Vous ne pouvez pas faire ça dans un véhicule !"];};
...
{
	case "jewelry": {[["diamondc","glass"],[1,2],"jewelry",1,"jewelry",20000];};
	case "microchips": {[["iron_r","copper_r"],[1,1],"microchips",2,"microchips",8000];};
	default {[]};
};
...
if (_productionAmount < 1) exitWith {
	...
	hint "Vous n'avez pas assez d'argent sur vous !";
	life_is_processing = false;
};
...
_pgText ctrlSetText format["%2 (1%1)...","%","Fabrication en cours"];
...
while{true} do
{
	...
	_pgText ctrlSetText format["%3 (%1%2)...",round(_cP * 100),"%","Fabrication en cours"];
	if(_cP >= 1) exitWith {};
	if(player distance _processor > 10) exitWith {};
};

if(player distance _processor > 10) exitWith {hint "Vous devez rester à moins de 10m de l'usine de fabrication !"; 5 cutText ["","PLAIN"]; life_is_processing = false;};
