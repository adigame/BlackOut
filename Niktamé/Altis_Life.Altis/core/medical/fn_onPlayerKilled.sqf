...
_unit spawn
{
	...
	_maxTime = time + (life_respawn_timer * 240); // 4 Minutes
	_RespawnBtn ctrlEnable false;
	waitUntil {_Timer ctrlSetText format["Réapparition disponible dans: %1",[(_maxTime - time),"MM:SS.MS"] call BIS_fnc_secondsToString]; 
	round(_maxTime - time) <= 0 OR isNull _this || life_request_timer};
	
	if (life_request_timer) then {
		
		_RespawnBtn ctrlEnable false;
		_maxTime = time + (life_respawn_timer * 700); // 7 Minutes after clicked button Call medic
		waitUntil {_Timer ctrlSetText format["Réapparition disponible dans: %1",[(_maxTime - time),"MM:SS.MS"] call BIS_fnc_secondsToString]; 
...
if(!isNull _killer && {_killer != _unit} && {side _killer != west} && {alive _killer}) then {
	if(vehicle _killer isKindOf "LandVehicle") then {
		[[getPlayerUID _killer,_killer getVariable["realname",name _killer],"13"],"life_fnc_wantedAdd",false,false] spawn life_fnc_MP;
